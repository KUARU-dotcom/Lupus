> Каноническая версия документа живёт в `main/docs`; ветка `specification` — замороженный архив.

# LUPUS V1.0 — Спецификация языка программирования

**Версия:** 1.0  
**Дата:** 2026-06-04  
**Статус:** Готов к реализации (Ready for Implementation)  
**Целевая аудитория:** LLM, разработчики интерпретатора, архитекторы компилятора  
...
10. [Чек-лист для LLM](#10-чек-лист-для-llm)
...
define        = LPAREN , "define" , IDENTIFIER , expr , RPAREN
              ...
              (* (define x 10) или (define (add a b) -> int (+ a b)) *)
...
define_public = LPAREN , "define-public" , func_header , body , RPAREN ;
              ...
              (* Обязательна аннотация возвращаемого типа и всех параметров. *)
              (* Параметры могут быть с аннотацией: (a int) или без: a *)
              (* Если параметр без аннотации в define-public, тип выводится, но тайп-чекер *)
              (* генерирует жесткую ошибку missing-param-type. *)

func_header   = LPAREN , IDENTIFIER , { param } , RPAREN , [ "->" , type_expr ] ;
              (* Имя функции + параметры + опциональная аннотация возвращаемого типа *)
              (* Для define-public отсутствие -> type_expr на этапе типизации — ошибка. *)
...
pattern       = "_"                              (* wildcard *)
              ...
              | "none"                           (* паттерн none — специальное правило, т.к. none — LITERAL_NONE, не IDENTIFIER *)
...
if_let        = LPAREN , "if-let" , LPAREN , IDENTIFIER , expr , RPAREN , expr , expr , RPAREN ;
              ...
              (* В v0.1 if-let работает только с Option. Для Result используйте match. *)
...
loop_expr     = LPAREN , "loop" , body , RPAREN ;
              ...
              (* Тип возвращаемого значения: unit. В v0.1 выход из loop только через (exit 0). Конструкций break/return нет. *)
...
type_expr     = "int" | "float" | "bool" | "str" | "unit"
              ...
              (* Встроенные параметризованные opaque-типы: (channel str) *)
```
...
- **Унарный минус:** Обрабатывается парсером как вызов `(- 0 x)`, а не на уровне лексера. Литералы отрицательных чисел в исходном коде не допускаются; используйте `(- 0 5)` для `-5` и `(- 0 3.5)` для `-3.5`.
...
- **Специальные встроенные формы `and` и `or`:** Формы `and` и `or` не являются обычными функциями. Они — специальные встроенные формы компилятора, поддерживающие короткое замыкание (ленивое вычисление) и принимающие 2+ аргументов. Например: `(and (> x 0) (< x 10))`. В грамматике они присутствуют в `operator` для синтаксического разбора, но семантически обрабатываются интерпретатором как специальные формы.
...
- **Параметризованные opaque-типы:** FFI-типы вроде `(channel str)` успешно разбираются существующим правилом парсера `LPAREN , IDENTIFIER , { type_expr } , RPAREN`. Для тайп-чекера `channel` — встроенный параметризованный непрозрачный (opaque) тип, аналогичный пользовательским дженерикам, но его внутреннее устройство скрыто.
- **Модификатор импорта `:all`:** Импортирует все символы `define-public` и экспорты FFI из целевого модуля напрямую в текущее окружение без префикса. Если импортируемое имя совпадает с уже существующим в текущем модуле, линтер/тайп-чекер обязан выдать ошибку `import-collision`.
...
|-----|---------|----------|------------|
| `int` | `42` | `+`, `-`, `*`, `/` (целочисленное деление), `%` (остаток), `=`, `!=`, `<`, `>`, `<=`, `>=` | 64-bit signed integer. Деление на 0 — runtime panic (`error: divide-by-zero`). Негативные числа в коде пишутся через унарный минус: `(- 0 7)`. |
| `float` | `3.14` | `+`, `-`, `*`, `/`, `=`, `!=`, `<`, `>`, `<=`, `>=` | 64-bit IEEE 754. Деление на 0 — `inf` / `-inf` (IEEE поведение), не ошибка. Негативные числа в коде пишутся через унарный минус: `(- 0 3.5)`. |
| `bool` | `true`, `false` | `not`, `=` | `not` — обычная функция. `and` и `or` — специальные формы с обязательным коротким замыканием. |
...
**Непрозрачные типы (opaque):** Типы вроде `socket`, `task`, `channel`, объявленные в FFI-модулях, также доступны в `type_expr` через `IDENTIFIER`. Их внутреннее устройство скрыто, операции с ними возможны только через функции модуля. Тип `channel` является встроенным параметризованным opaque-типом: `(channel str)`.
...
| `=` | `(Func a a bool)` | Равенство. Работает для всех типов, кроме `Func`. Для opaque типов (socket, task, channel) — сравнение по идентичности объекта. | `(= 1 1)` → `true` |
...
**Примечание:** `and` и `or` не включены в таблицу выше, так как они являются специальными встроенными формами с коротким замыканием (см. раздел 1.3).
...
**Типы `task` и `channel`:** Opaque types. Создаются только через `amiya`. Тип `channel` является встроенным параметризованным opaque-типом: `(channel str)`.
...
Форма `assert` является **встроенной специальной формой** языка (не функцией модуля `core`), которая при `false` вызывает runtime panic. Форма `test` является **специальной формой** языка (встроенной в парсер), а не функцией. Импорт модуля `w` не требуется для использования `(test ...)` или `(assert ...)`. Однако функции `test/assert-eq`, `test/assert-true`, `test/assert-false` доступны после `(import (w test))` или через префикс `test/`.
...
| `missing-return-type` | type | Отсутствует аннотация возвращаемого типа у public-функции | `(define-public (foo x) x)` — жесткая ошибка |
| `missing-param-type` | type | Отсутствует аннотация типа параметра у public-функции | `(define-public (foo x) -> int x)` — жесткая ошибка |
...
| `duplicate-struct-field` | type | Дублирование имени поля в структуре | `(defstruct p (x int) (x float))` |
| `import-collision` | lint | Конфликт имён при импорте `:all` | `(import (senko math) :all)` при существующем `pi` |
...
| `PatternLiteralNone` | `{}` | Паттерн для `none` |
...
- `define-public` требует `func_header` (имя в скобках с параметрами) и `body`. Парсер допускает опускание аннотаций в `func_header` (согласно EBNF), но тайп-чекер обязан сгенерировать жесткие ошибки `missing-return-type` и `missing-param-type` для любого `define-public` без полной сигнатуры.
- Параметры в `define-public` могут быть с аннотацией `(a int)` или без (`a`). Если без — тип выводится, но тайп-чекер выдает ошибку `missing-param-type`.
...
- `defstruct` поддерживает дженерики: `(defstruct (Node a) ...)`. Поля структуры должны иметь уникальные имена; дублирование — ошибка `duplicate-struct-field`.
...
**Поведение `set!` и типовая безопасность:**
- Форма `set!` **не изменяет** тип переменной и **не обобщает** его.
- Если переменная была объявлена как `int`, попытка выполнить `(set! x "str")` вызывает жесткую ошибку `type-mismatch`, так как ожидаемый тип `int` не унифицируется со `str`.
- `set!` ищет переменную в текущем и родительских окружениях; изменяет первую найденную mutable. Если переменная найдена, но не mutable — ошибка `immutable-assignment`.
...
- Если `define-public` без полной аннотации (возвращаемый тип или любой параметр без типа) — жесткие ошибки `missing-return-type` или `missing-param-type`.
...
### 10.3. Порядок генерации (рекомендация для LLM)
...
*Документ подготовлен для LLM. Все разделы самодостаточны и содержат достаточно информации для реализации языка Lupus V1.0 на Python.*